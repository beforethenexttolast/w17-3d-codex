...
| `w17-mechanical-measurement-session-prompt.md` | the owner's no-power caliper session (M-rows) — **the record and the reasoning**; measured values flow into `w17_params.scad`, never the reverse |
| `MEASUREMENT_SITTING_RUNBOOK.md` + `MEASUREMENT_RECORD_SHEET.{md,csv}` | the **executable order** for that same sitting (stations, tools, stop conditions) and the bench **capture** sheets. They hold no values of their own: captured numbers are transcribed into the prompt, then `w17_params.scad`, then AA §12. **The two sheets are GENERATED — never hand-edit them**; `tools/gen_measurement_sheet.py <repo-root>` writes both from one row list |
| `tools/gen_measurement_sheet.py` | the single source of both record sheets. Its `param` column is the mapping into `w17_params.scad` and is what `11_cad/tools/ingest_measurements.py` reads; a cell with **no** `param` is register-only and its number is **not** transcribed into the parameter file. Invariant: **one row per parameter**, and a measured **stock** dimension is never written into the **feature** parameter it must fit (feature = stock + clearance, a CAD decision) |
