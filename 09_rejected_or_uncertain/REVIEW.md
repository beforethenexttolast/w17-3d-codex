...
| `Rear Back Motor Cover REVISION 1.stl` | `unsorted_stl_raw/RC-01 Revision 1.1/Original + Revision 1 Files pre-released to fully build the car/RC-01 Revision 1 Files/Rear Axle Upgrades` | ASA if selected | medium | GATE A (restated 2026-07-10): check the selected rear rocker / spring-mount / rear-stack configuration correctly seats AND articulates with the **68 mm rear shocks** (rear length confirmed; 51/52 mm are front-only). Fits → Rev-1 stack (bearings seat in these covers per [7] — check whether Left/Rightrearaxle are replaced), else original stack; resolve together with the rear-wing gate; diagnostic TP prints allowed |
...
| `camera_blower_duct.scad` | `unsorted_stl_raw` | PETG | high | GATE C (research/design task): current duct source/design candidate — inspected 2026-07-10, fully PARAMETERIZED (9 "MEASURE THESE" dims + wall/clearance + optional M3 tab), shipped values are placeholder defaults. Diagnostic default-render → temp STL → Bambu Studio visual check is allowed (design validation only). Production blocked until: camera measured (calipers), blower arrived + measured, placement decided, airflow path / lens clearance / service access checked — checklist FIRST_PRINT_DECISION.md §6 |
