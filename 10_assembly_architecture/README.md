...
| [`P_assembly_master_manual.md`](P_assembly_master_manual.md) | Dependency-aware assembly manual **ASM-01…ASM-60**, including the generated joint-by-joint closure appendix — authoritative for assembly order |
| [`Z_connection_joint_register.md`](Z_connection_joint_register.md) / [CSV](Z_connection_joint_register.csv) | **Project-wide connection execution register**: one row per physical joint, BOM hardware, measured/documented/assumed interface, retention, dependency, tool, torque/threadlock note, ASM check and DEFER state |
...
| [`AA_electronics_placement_study.md`](AA_electronics_placement_study.md) | **(2026-09-03, mechanical takeover)** the single "what goes where" answer for every onboard module, the **second-floor board cage** concept, the DRS linkage check, USB-C charge-flap candidates, the GCS-box concept, the ordered measurement list (M-00…M-20) and the ASSUMED-parameter list feeding [`../11_cad/`](../11_cad/README.md). Continues the letter series at `AA` because `X` is spent on the diagnostic-CAD review below. **Proves no fit** — every statement is a gauge at S0 = 0 against the provisional KO-01 |
| [`fit_studies/`](fit_studies/README.md) | **(2026-07-23/24)** eight remaining-component studies plus the separate ZK lift-out-cassette physical audit, using the Y confidence/datum discipline; steering is referenced, not repeated |
| [`viz/index.html`](viz/index.html) | **Self-contained HTML master** linking eight theme-aware, to-scale zone visualizations plus the separate cassette audit set; generated by `evidence/scripts/p0_08_zone_visualizations.py` and `p0_12_cassette_fit_visualizations.py` |
| [`viz/connections/index.html`](viz/connections/index.html) | **Self-contained exploded connection atlas** linking twelve theme-aware, to-scale assembly sheets with real STL silhouettes and technical hardware outlines; generated by `evidence/scripts/p0_10_connection_visualizations.py` |
| [`evidence/scripts/p0_10_connection_visualizations.py`](evidence/scripts/p0_10_connection_visualizations.py) / [`p0_11_validate_connection_outputs.py`](evidence/scripts/p0_11_validate_connection_outputs.py) | Regenerates the connection register/manual block/HTML atlas and validates cross-output Joint-ID, deferral, link, scale, theme and asset coverage |
| [`viz/cassette/index.html`](viz/cassette/index.html) | **Separate cassette fit-audit visualization set**: real-shell plan/side/section layout, exploded cassette and routed ganged umbilical; generated/validated by `p0_12_cassette_fit_visualizations.py` / `p0_13_validate_cassette_outputs.py`; emits no STL |
...
CAD hand-off → `T` · physical neighbour connections → `Z` (generated into the
joint-controlled appendix of `P`). Reports A–C and F–H are frozen
evidence/selection records.
